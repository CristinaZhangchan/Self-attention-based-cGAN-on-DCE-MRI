...
# --- 修改开始: 自动适配 scikit-image 版本 ---
try:
    # 新版本 (scikit-image >= 0.19)
    from skimage.metrics import peak_signal_noise_ratio as psnr
except ImportError:
    # 旧版本 (scikit-image < 0.19)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        if epoch % opt.save_epoch_freq == 0:
            ...
            for i, data_val in enumerate(dataset_val):
                ...
                # 兼容性调用
                try:
                    psnr_avg[epoch-1, i] = psnr(fake_im / fake_im.max(), real_im / real_im.max(), data_range=1.0)
                except TypeError:
                     # 旧版参数可能不同，做个简单的 fallback
